...
/**
 * 패키지명이 틀리면 so 파일 로드는 되지만 라이선스가 안됨
 * 꼭 com.inzisoft.pdf2image 에 둬야함.  <----- 중요.
 * 
 * @author ppusari
 *
 */
public class InziPDF {
	private static final Logger log = LoggerFactory.getLogger(InziPDF.class);
	 ...
	 * @return
	 */
	public static int init(String jniFile, String licenseFile) {
		
		int ret = 0;
		
		log.info("jniFile: " + jniFile);
		log.info("licenseFile:= " + licenseFile);
		
		File file = new File(licenseFile);			
		if (file.exists()) {
			System.load(jniFile);
			int result = loadLicenseFile(licenseFile);
			
			if (result == 1) {
				log.info("======================");
				log.info("pdf image so file SUCCESS");
				log.info("======================");
				status = true;
				ret = result;
			}
				...
				log.error("======================");
				log.error("pdf image license fail");
				log.error("======================");				
				ret = result;
			}
		}				
			...
			log.error("======================");
			log.error("pdf image so file fail");
			log.error("======================");
			ret = 0;
		}
...
//		
//		try {			
//			System.load("/programs/app/socr/engine/modules/libJNI_InziPDF2Image.so");
//			loadLicenseFile("/home/weblogic/inzisoft/socr/license/license.xml");
...
//	}
	...
	public static int getPDFPageCountEx(String filenameSrc) {
		if (status == true) {
			return getPDFPageCount(filenameSrc);
		}
	 ...
	 * @param comprate    [in] 압축을 사용하는 포맷의 압축 값<br/>
	 *                    JPEG, JPEG in TIFF: 1 ~ 100<br/>
	 *                    JPEG2000, JPEG2000 in TIFF: 24 - 1bpp, 12 - 2bpp, 48 -
	 *                    0.5bpp<br/>
	 * @param filetype    [in] 저장할 파일 형식 설정<br/>
	 *                    1: BMP<br/>
	 *                    2: JPEG<br/>
	 *                    3: JPEG2000<br/>
	 *                    4: TIFF<br/>
	 * @param comptype    [in] filetype이 TIFF일 때 압축 방식 설정<br/>
	 *                    1: 무압축<br/>
	 *                    2: RLE<br/>
	 *                    3: G3<br/>
	 *                    4: G4<br/>
	 ...
	 *                    34713: JPEG2000<br/>
	 *                    34663: JBIG2<br/>
	 * @param binarize    [in] 이진화 여부 설정<br/>
	 *                    0: 이진화 수행 안 함<br/>
	 *                    그외: 이진화 수행<br/>
	 *                    filetype이 1, 4일 사용 가능<br/>
	 *                    filetype이 4이고, comptype이 3, 4, 34663 일 때 사용 가능<br/>
	 * @param threshold   [in] binarize가 1일 때 이진화 기준값 설정, 0일 경우 내부적으로 적당한 기준값을 사용하여
	 *                    이진화
	 * @return 1: 성공<br/>
	 *         그외: 실패<br/>
	 */
	 ...
	 * @param comptype     [in] filetype이 TIFF일 때 압축 방식 설정, 상세 내용은 convertPDF2Image
	 *                     참조
	 * @param binarize     [in] 이진화 여부 설정, 상세 내용은 convertPDF2Image 참조
	 * @param threshold    [in] binarize가 1일 때 이진화 기준값 설정, 0일 경우 내부적으로 적당한 기준값을 사용하여
	 *                     이진화
	 * @param makeizt      [in] InziForm 형식의 썸네일 이미지를 생성 설정<br/>
	 *                     0: 생성 안 함<br/>
	 *                     그외: 생성<br/>
	 ...
	 *         그외: 실패<br/>
	 */
	public static native int convertPDF2ImageOnePage(String filenameSrc, String folderDst, int resolution, int comprate,
			int filetype, int comptype, int binarize, int threshold, int page); // page
																				// =
																				// 0-based
	 ...
	 * @param filenameSrc [in] 파일 이름
	 * @param page        [in] 변환할 이미지의 페이지 번호 (0-base)
	 * @param sizemode    [in] 이미지 크기 지정 방식<br/>
	 *                    1: DPI모드<br/>
	 *                    2: Pixel모드<br/>
	 * @param size        [in] 이미지 크기의 값<br/>
	 *                    sizemode가 1일 경우: 100, 200, 300등 DPI값<br/>
	 *                    sizemode가 2일 경우: 3509등 pixel값<br/>
	 * @param datamode    [in] raw 이미지의 형식<br/>
	 *                    0: RGBA8888<br/>
	 *                    1: RGB565<br/>
	 *                    2: RGB<br/>
	 *                    3: DIB<br/>
	 * @param aDataInfo   [out] 결과 값<br/>
	 ...
	 * @param filenameSrc          [in] 파일 이름
	 * @param filenameDst          [in] 추출될 파일 이름
	 * @param extractPages         [in] 추출할 페이지 번호들(1-base) - 각 페이지 번호는 구분자로 구분 -
	 ...
	 *         그외: 실패<br/>
	 */
	public static native int extractPDFPages(String filenameSrc, String filenameDst, String extractPages, int delimiter,
	 ...
	 * @param filenameSrc          [in] 파일 이름
	 * @param filenameDst          [in] 추출될 파일 이름
	 * @param extractPages         [in] 제거할 페이지 번호들(1-base) - 각 페이지 번호는 구분자로 구분
	 * @param delimiter            [in] 페이지 구분자
	 * @param skipStreamComparison [in] PDF stream 비교를 스킵 할 지 여부
	 * @return 1: 성공<br/>
	 *         그외: 실패<br/>
	 ...
	 * @param reload [in] 0 - reload 안함, 1 - reload, 1로 설정할 경우 해당 페이지가 로드 되어 있어도 다시
	 *               재로드
	 * @return 1: 성공<br/>
	 *         그외: 실패<br/>
	 */
	 ...
	 * 
	 * @param pdfdoc       [in] PDF 문서 포인터
	 * @param searchedPage [out] 각 페이지별 텍스트 검색 여부 (데이터의 크기 >= 페이즈 수)
	 ...
	 * @param searchedInfo [out] 검색된 텍스트 정보<br/>
	 ...
	 * @param transformData [out] 변환된 좌표<br/>
