...
/**
 * <p>A registered {@code IANA} mime media types.
 * 
 * @author Kisong.Kim
 * @version 1.0
 * @since 1.0
...
public enum MediaTypes {
	...
	public static MediaTypes fromExtension(String s) {
		for (MediaTypes c : MediaTypes.values()) {
			if (c.extension.equalsIgnoreCase(s)) {
				return c;
			}
		}
		return DEFAULT;
	...
	public static MediaTypes fromFileName(String fileName) {
		...
		String s = "";
		if(lastIndex > 0) {
			s = fileName.substring(lastIndex+1).toLowerCase();
		} else {
			return DEFAULT;
		}
	...
	public static MediaTypes fromMime(String s) {
		for (MediaTypes c : MediaTypes.values()) {
			if (c.mime.equalsIgnoreCase(s)) {
				return c;
			}
		}
		return DEFAULT;
	...
	public static boolean isImageFromExtension(String s)
	{
	...
	public static boolean isAudioFromExtension(String s)
	{
		MediaTypes c = fromExtension(s);
